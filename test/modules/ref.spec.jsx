...
describe('ref模块', function() {
   ...
   */
	...
	it('should remove refs when destroying the parent', () => {
		...
		expect(Object.keys(testInstance.refs || {}).filter((key) => testInstance.refs[key]).length).toEqual(1);
		ReactDOM.unmountComponentAtNode(container);
		expect(Object.keys(testInstance.refs || {}).filter((key) => testInstance.refs[key]).length).toEqual(0);
	});
	...
	it('should remove refs when destroying the child', () => {
		...
		expect(Object.keys(testInstance.refs || {}).filter((key) => testInstance.refs[key]).length).toBe(1);
		ReactDOM.render(<TestComponent destroy={true} />, container);
		expect(Object.keys(testInstance.refs || {}).filter((key) => testInstance.refs[key]).length).toBe(0);
	});
	...
	it('Stateless组件的ref不会被执行', function() {
		var b;
		function App() {
	...
	it('相同位置上的元素节点的string ref值不一样', function() {
		class Foo extends React.Component {
			render() {
				return (
					<div>{this.props.a ? <span ref="aaa" className="aaa" /> : <span ref="bbb" className="bbb" />}</div>
				);
			}
		}
	...
	it('相同位置上的元素节点的ref函数一样', function() {
		...
		class Foo extends React.Component {
			render() {
				return (
					<div>{this.props.a ? <span ref={refFn} className="aaa" /> : <b ref={refFn} className="bbb" />}</div>
				);
			}
		}
		ReactDOM.render(<Foo a={1} />, div);
