(function umd(root, factory) {
    ...
    function findAllInRenderedTree(inst, test) {
        ...
        var ret = dom && dom.nodeType === 1 && test(dom) ? [dom] : [];
        if (inst.vtype === 1) {
        ...
        } else if (inst.vtype || inst.__rendered) {
            var rendered = inst._instance ? inst._instance.__rendered : inst.__rendered;
            if (rendered) {
                //如果是实例
    ...
    "click,change".replace(/\w+/g, function(name){
        ReactTestUtils.Simulate[name] = function(node){
            var fakeNativeEvent = {};
            fakeNativeEvent.target = React.findDOMNode(node);
            fakeNativeEvent.simulated = true;
            React.eventSystem.dispatchEvent(fakeNativeEvent, name);
        };
    });
