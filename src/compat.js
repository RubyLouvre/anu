...
let IEHandleFix = {
    ...
    change: function (dom) {
        ...
        let mask = /radio|check/.test(dom.type) ? "click" : /text|password/.test(dom.type) ? "propertychange" : "change";
    ...
    extend(
        ...
        oneObject("mousemove, mouseout,mouseenter, mouseleave, mouseout,mousewheel, mousewheel, whe" + "el, click", function (event) {
            if (!("pageX" in event)) {
                ...
                let box = doc.compatMode === "BackCompat" ? doc.body : doc.documentElement;
