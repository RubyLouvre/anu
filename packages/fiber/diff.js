...
export function render(vnode, root, callback) {
    ...
    Renderer.updateComponent(rootInstance, {
        child: vnode
    }, wrapCb(callback));
...
Renderer.updateComponent = function (instance, state, callback) {
    ...
    if (fiber.willing) {
        ...
        //batchedUpdate(...)
        if (isBatchingUpdates) {
            pushChildQueue(fiber, batchedqueue, true);
        } else {
            //如果在回调中hydrating为true
