...
import { guardCallback, pushError } from './ErrorBoundary';
...
export function updateClassComponent(fiber, info) {
	...
	let rendered = guardCallback(instance, 'render', [], true);
...
const stageIteration = {
	mount(fiber, nextProps, nextContext, instance) {
		...
		if (instance.__useNewHooks) {
			...
			callUnsafeHook(instance, 'componentWillMount', [], true);
		}
	},
	receive(fiber, nextProps, nextContext, instance, contextStack) {
		...
		if (instance.__useNewHooks) {
			...
			if (willReceive) {
				fiber.setout = true;
				callUnsafeHook(instance, 'componentWillReceiveProps', [nextProps, nextContext], true);
				return 'update';
			} else {
				cloneChildren(fiber);
				return false;
	...
	update(fiber, nextProps, nextContext, instance) {
		...
		fiber._hydrating = true;
		if (!fiber.isForced && !guardCallback(instance, 'shouldComponentUpdate', args, true)) {
			cloneChildren(fiber);
		} else if (!instance.__useNewHooks) {
			callUnsafeHook(instance, 'componentWillUpdate', args, true);
		}
	},
};

function callUnsafeHook(a, b, c) {
	guardCallback(a, b, c, true);
	guardCallback(a, 'UNSAFE_' + b, c, true);
}
