...
module.exports = {
	...
	FunctionDeclaration: {
		...
		exit(astPath, state) {
			...
			if (/^[A-Z]/.test(name) && modules.componentType === 'Component' && !modules.parentName) {
				//需要想办法处理无状态组件
				helpers.render.exit(astPath, '无状态组件', name, modules);
				modules.createPage = utils.createRegisterStatement(name, name);
			}

			if (astPath.parentPath.type === 'ExportDefaultDeclaration' && modules.componentType === 'Component') {
				astPath.node.body.body.unshift(template(utils.shortcutOfCreateElement())());
			}
				...
				astPath.insertBefore(
					t.variableDeclaration('var', [
						t.variableDeclarator(
							...
							t.callExpression(t.identifier('require'), [t.stringLiteral('regenerator-runtime/runtime')])
						),
					])
				);
			}
		...
		exit(astPath) {
			...
			switch (declaration.type) {
				...
				case 'VariableDeclaration':
					...
					astPath.replaceWithMultiple([declaration, utils.exportExpr(id)]);
					break;
				case 'FunctionDeclaration':
					astPath.replaceWithMultiple([declaration, utils.exportExpr(declaration.id.name)]);
					break;
				case '{}':
	...
	ClassProperty: {
		exit(astPath, state) {
			...
			if (key === 'config') {
				//format json
				var str = generate(astPath.node.value).code;
				try {
                    var json = eval('0,' + str);
                    Object.assign(modules.config, json);
				} catch (e) {
					console.log('生成json有问题',str);
				}
			} else if (astPath.node.static) {
				var keyValue = t.ObjectProperty(t.identifier(key), astPath.node.value);
				...
				if (key == 'globalData' && modules.componentType === 'App') {
					...
					astPath.node.value.properties.push(
						t.objectProperty(t.identifier('buildType'), t.stringLiteral(config.buildType))
					);
	...
	JSXOpeningElement: {
		enter: function(astPath, state) {
			...
			if (bag) {
				...
				attributes.push(
					t.JSXAttribute(t.JSXIdentifier('is'), t.jSXExpressionContainer(t.stringLiteral(nodeName)))
				);
				...
				if (modules.indexArr) {
					...
					attributes.push(
						utils.createAttribute(
							'$$index',
							t.jSXExpressionContainer(t.identifier(modules.indexArr.join("+'-'+")))
						)
					);
				}
				//处理组件标签的内部，现在支持slot
		...
		enter: function(astPath, state) {
			...
			if (t.isJSXExpressionContainer(attrValue)) {
				...
				if (/^(?:on|catch)[A-Z]/.test(attrName)) {
					...
					var otherEventName = utils.getEventName(eventName, nodeName, buildType);
					...
					attrs.push(utils.createAttribute(name, 'e' + utils.createUUID(astPath)));
					...
					if (isIdentifier || MemberExpression || styleType === 'ObjectExpression') {
						...
						var styleRandName = `'style${utils.createUUID(astPath)}'` + (ii ? ' +' + ii : '');
						...
						attrs.push(
							utils.createAttribute(
								'style',
								t.jSXExpressionContainer(
									t.identifier(`React.toStyle(${styleName}, this.props, ${styleRandName})`)
								)
							)
						);
						astPath.remove();
	...
	JSXClosingElement: function(astPath, state) {
		...
		if (!modules.importComponents[nodeName] && nodeName !== 'React.useComponent') {
			helpers.nodeName(astPath, modules);
		} else {
			astPath.node.name.name = 'React.useComponent';
		}
