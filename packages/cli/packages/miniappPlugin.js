...
module.exports = {
	...
	ExportNamedDeclaration: {
		exit(astPath) {
			...
			if (!declaration) {
				...
				astPath.replaceWith(utils.exportExpr(declaration.name, declaration.name));
	...
	JSXOpeningElement: {
		enter: function(astPath, state) {
			...
			if (modules.importComponents[nodeName]) {
				...
				attributes.push(
					utils.createAttribute('$$loop', 'data' + utils.createUUID(astPath)),
					t.JSXAttribute(t.JSXIdentifier('is'), t.jSXExpressionContainer(t.identifier(nodeName)))
				);
	...
	JSXClosingElement: function(astPath, state) {
		...
		if (!modules.importComponents[nodeName] && nodeName !== 'React.toComponent') {
			helpers.nodeName(astPath, modules);
		} else {
			astPath.node.name.name = 'React.toComponent';
		}
