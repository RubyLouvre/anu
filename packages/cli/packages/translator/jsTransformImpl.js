  ...
  JSXOpeningElement: {
    enter: function(astPath, state) {
      ...
      if (modules.importComponents[nodeName]) {
        ...
        if (!isEmpty) {
          astPath.fragmentUid = 'f' + astPath.node.start + astPath.node.end;
          ...
          attributes.push(
            ...
            utils.createAttribute('fragmentUid', astPath.fragmentUid)
          );
        }
      } else {
    ...
    if (/^(?:on|catch)[A-Z]/.test(attrName)) {
      ...
      if (!attrs.setClassCode) {
        ...
        if (keyValue != undefined) {
          attrs.push(t.JSXAttribute(t.JSXIdentifier('data-key'), keyValue));
        }
      }
