...
module.exports = {
	...
	ExportNamedDeclaration: {
		...
		exit(astPath) {
			...
			if (!declaration) {
				...
				astPath.replaceWith(helpers.exportExpr(declaration.name, declaration.name));
			} else if (declaration.type === 'VariableDeclaration') {
				var id = declaration.declarations[0].id.name;
				declaration.kind = 'var'; //转换const,let为var
				astPath.replaceWithMultiple([declaration, helpers.exportExpr(id)]);
	...
	CallExpression: {
		enter(astPath, state) {
			...
			if (
				(t.isJSXExpressionContainer(astPath.parentPath) || t.isConditionalExpression(astPath.parentPath)) &&
	...
	JSXOpeningElement: {
		enter: function(astPath, state) {
			...
			if (modules.importComponents[nodeName]) {
				...
				attributes.push(
					utils.createAttribute('templatedata', 'data' + utils.createUUID()),
					t.JSXAttribute(t.JSXIdentifier('is'), t.jSXExpressionContainer(t.identifier(nodeName)))
				);
				if (!isEmpty) {
					//创建
					var fragmentUid = 'f' + astPath.node.start + astPath.node.end;
	...
	JSXAttribute: function(astPath, state) {
		...
		if (/^(?:on|catch)[A-Z]/.test(attrName)) {
			...
			var styleRandName =
				`"style${astPath.node.start + astPath.node.end}"` + (modules.indexName ? ' +' + modules.indexName : '');
			if (styleType === 'Identifier') {
				...
				attrs.push(
					t.JSXAttribute(
						t.JSXIdentifier('style'),
						t.jSXExpressionContainer(
							t.identifier(`React.collectStyle(${styleValue}, this.props, ${styleRandName})`)
						)
					)
				);
				astPath.remove();
