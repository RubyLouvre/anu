...
module.exports = {
  ...
  ExportNamedDeclaration: {
    ...
    exit(path) {
      ...
      if (!declaration) {
        ...
        path.replaceWith(helpers.exportExpr(declaration.name, declaration.name));
  ...
  JSXAttribute: function(path, state) {
    ...
    if (attrName === 'style' && t.isJSXExpressionContainer(path.node.value)) {
      
      var styleType = path.node.value.expression.type;
      
      var styleRandName = 'style' + jsx.createUUID();
      if (styleType === 'Identifier') {
        // 处理形如 <div style={formItemStyle}></div> 的style结构
        var styleName = path.node.value.expression.name;
        attrs.push(
          t.JSXAttribute(
      ...
      if (styleType === 'ObjectExpression') {
        ...
        var styleValue = generate(path.node.value.expression).code;
        attrs.push(
          t.JSXAttribute(
            t.JSXIdentifier('style'),
            t.jSXExpressionContainer(
