// 处理有狀态组件与纯组件与createContext创建的两种组件
...
export function updateClassComponent (current, fiber, Component, nextProps, renderExpirationTime) {
  ...
  if (instance === null) {
    ...
    constructClassInstance(fiber, Component, nextProps)
  ...
  const child = finishClassComponent(current, fiber, Component, shouldUpdate, hasContext, renderExpirationTime)
...
function constructClassInstance (fiber, ctor, props) {
  ...
  if (Object(contextType) === contextType) {
    // React 16.7的static contextType
...
function updateClassInstance (current, fiber, ctor, newProps, renderExpirationTime) {
  ...
  instance.props = fiber.type === fiber.type ? oldProps : resolveDefaultProps(fiber.type, oldProps)
