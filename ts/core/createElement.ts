...
function hasValidRef(config: Ref):boolean {
    return config.ref !== undefined;
}

function hasValidKey(config: Ref):boolean {
...
export function createElement(type: string | Function, config: Object|null, ...children: any ) {
    let props = {},
        tag = 5,
        key = null,
        ref = null,
...
function ReactElement(type: Function | string, tag:ComponentType, props:{}, 
    key?: string | null, ref?: any, owner?: null| {}): ReactElement {
...
export function createVText(text: string | number): ReactElement {
    return ReactElement('#text', 6, text + '');
}

function escape(key:Simple):string {
    const escapeRegex = /[=:]/g;
    const escaperLookup = {
        '=': '=0',
        ':': '=2'
...
function getComponentKey(component, index :number):string {
